...
async function negotiateValuesForApiKeyScheme(
  ...
  const value = await prompt.maskedText({
    message: 'Please insert secret for API Key (leave empty to use env variable):',
  });
...
async function negotiateValuesForHttpScheme(
  ...
  switch (resolvedSecurityScheme.securityScheme.scheme) {
    case 'basic':
      ...
      break;
    case 'bearer':
      await negotiateBearerToken(ctx, config);
      break;
    case 'digest':
    default:
      throw new Error('Unexpected security scheme');
...
async function negotiateBearerToken(ctx: Context, { userConfig, serverClientConfig }: SecurityConfig): Promise<void> {
  const value = await prompt.maskedText({
    message: 'Please insert token (leave empty to use env variable):',
  });
...
async function negotiateValuesForOauth2Scheme(
  ctx: Context,
  config: SecurityConfig,
  _resolvedSecurityScheme: ResolvedSecurityScheme,
): Promise<void> {
  await negotiateBearerToken(ctx, config);
}
