...
import type { TAgentMcpServerId, TMcpServerId } from '@libs/db-ids';
...
export const ServerRow = memo(function ServerRow({ server, agentServerId, isActive }: ServerRowProps) {
  ...
  return (
    <Link
      to="."
      search={prev => {
        let nextAgentServerId = agentServerId;
        if (prev.agentServerId === agentServerId) {
          nextAgentServerId = undefined;
        }

        let nextServerId = server.id as TMcpServerId | undefined;
        if (prev.serverId === server.id && prev.agentServerId === agentServerId) {
        ...
        return {
          ...
          agentServerId: nextAgentServerId,
        };
      }}
      className={tn(
